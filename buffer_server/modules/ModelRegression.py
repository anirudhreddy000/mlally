...
class ModelSelector:
    ...
    def fit(self, X_train, y_train):
        for name, pipeline in self.pipelines.items():
            try:
                ...
                print(f"Process interrupted during GridSearchCV for {name}. Exiting...")
                return
            except Exception as e:
                print(f"An error occurred during GridSearchCV for {name}: {e}")
                continue
    ...
    def save_best_model(self, file_path):
        if self.best_model is not None:
            ...
            print("No model has been fitted yet, so nothing to save.")